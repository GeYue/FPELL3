...
import ast, re, gc, copy, sys, random, os, math, time, glob
...
class CFG:
    ...
    print_freq=200 #40
    num_workers=52
    model='model/huggingface-bert/deberta-v3-base' #"microsoft/deberta-v3-large" #"microsoft/deberta-v3-base"
    ...
    min_lr=1e-8 #1e-6/7 for others 1e-8 for DebertaV2 XL/XXL
    eps=1e-8
    betas=(0.9, 0.999)
    batch_size=12 #64 #24 #12
    max_len=768 #1000 #512
    weight_decay=0.01
    ...
    lr_decay_cofficient=0.9 # 0.9(12 Layers) 0.9508(24 Layers) 0.9756(48 Layers)

    pooling = 'mean' # mean, max, min, attention, attention_superior, weightedlayer
    layer_start = 9 #9 
...
class CustomModel(nn.Module):
    def __init__(self, cfg, config_path=None, pretrained=False):
        ...
        #     self.pool = MinPooling()
            ...
            weights = [2**-i for i in range(0, self.config.num_hidden_layers-CFG.layer_start+1)]
            weights.reverse()
    ...
    def forward(self, inputs):
        ...
        if self.cfg.pooling == 'attention_sup':
            return feature
        else:
    ...
    for step, (inputs, labels) in enumerate(train_loader):
        ...
        print('Epoch: [{0}][{1}/{2}] '
                  ...
                  .format(epoch+1, step, len(train_loader), 
                          ...
                          loss=losses,
                          grad_norm=grad_norm,
                          lr=scheduler.get_last_lr()[0]), end='', flush=True)
...
def valid_fn(valid_loader, model, criterion, device):
    ...
    for step, (inputs, labels) in enumerate(valid_loader):
        ...
        print('EVAL: [{0}/{1}] '
                  'Elapsed {remain:s} '
                  'Loss: {loss.val:.4f}({loss.avg:.4f}) \r'
                  .format(step, len(valid_loader),
                          loss=losses,
                          remain=timeSince(start, float(step+1)/len(valid_loader))), end='', flush=True)
...
def train_loop(folds, fold):
    ...
    pseudo_file = glob.glob(f"./kaggle/043/rumor03/*{CFG.model.split('/')[-1]}_gpu{GPU}_fold{fold}*.csv")
    pseudo_df = pd.read_csv(pseudo_file[0])
    print (f"Reading pseudo lable files:: {pseudo_file[0]}")
    pseudo_df = pd.merge(pseudo_df, test, on='text_id')
    pseudo_df['fold'] = fold
    columns = ['text_id', 'full_text', 'cohesion', 'syntax', 'vocabulary', 'phraseology', 'grammar', 'conventions', 'fold']
    ##!!!pseudo_df.columns = columns
    pseudo_df = pseudo_df[columns]
    train_folds = pd.concat([train_folds, pseudo_df])
